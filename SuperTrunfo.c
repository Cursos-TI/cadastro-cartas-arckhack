...
int main() {
    ...
    // Novas variáveis calculadas da Carta 1
    float densidade1;
    float pib_percapta1;
    ...
    // Novas variáveis calculadas da Carta 2
    float densidade2;
    float pib_percapta2;
   ...
   // Saída formatada (mostrando os dados)
    ...
    //Nivel Novato: Incluí a comparação de um atributo das cartas como solicitado. 
    printf("\nComparação de Cartas (Atributo: PIB):\n"); //O atributo escolhido foi o PIB das cidades.
    printf("Carta 01 - %s: PIB: %.2f bilhões\n", cidade1, pib1);
    printf("Carta 02 - %s: PIB: %.2f bilhões\n", cidade2, pib2);
    if (pib1 > pib2) {
        printf("O PIB da Carta 01 venceu!\n");
    } else if (pib2 > pib1) {
        printf("O PIB da Carta 02 venceu!\n");
    } else {
